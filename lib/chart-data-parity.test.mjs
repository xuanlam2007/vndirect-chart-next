...
test("aligns overlay updates to main bars without filling comparison gaps", () => {
  const mainBars = [100, 200, 300, 400].map((time) => ({
    time, open: 1, high: 1, low: 1, close: 1, volume: 1,
  }));
  assert.deepEqual(
    alignOverlayToMainBars(mainBars, [
      { time: 150, value: 10 },
      { time: 175, value: 11 },
      { time: 350, value: 20 },
      { time: 450, value: 30 },
...
test("counts down to the current bar close", () => {
  assert.equal(barCloseCountdown(100, "1", 125), "00:35");
  assert.equal(barCloseCountdown(100, "60", 125), "59:35");
  assert.equal(barCloseCountdown(100, "1", 160), null);
});
...
test("normalizes VNDIRECT socket ticks and rejects invalid payloads", () => {
  ...
  assert.equal(normalizePriceTick({ symbol: "VN30", price: "1942.12" }), undefined);
